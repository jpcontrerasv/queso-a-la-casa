			...
			class BE_Table_Rate_Shipping extends WC_Shipping_Method {
				...
				*/
				...
				public function admin_options() {
			        ...
			        $zones = be_get_zones();
               		foreach($zones as $val) $zoneOpsJS .= "<option value=\"".$val['zone_id']."\">".$val['zone_title']."</option>";
	?>
...
?>
	...
	<?php
				...
				function process_table_rates() {
					...
					for ( $i = 0; $i <= $key; $i++ ) {
						if(isset($class_scp[$i])) {
							...
							$class_priorities[sanitize_title($class_scpid[$i])] = array(
								...
								'excluded' => ( $class_excluded[ $i ] ) ? $class_excluded[$i] : FALSE
							);
						}
					}
					...
					for ( $i = 0; $i <= $key; $i++ ) {
						if ( isset( $table_rate_title[$i] ) && isset( $table_rate_zone[$i] ) && isset( $table_rate_cond[$i] ) ) {
							...
							$table_rate_cost[$i] = woocommerce_format_total($table_rate_cost[$i]);//number_format($table_rate_cost[$i], 2,  '.', '');
							$table_rate_min[$i] = woocommerce_format_total($table_rate_min[$i]);//number_format($table_rate_min[$i], 2,  '.', '');
							if($table_rate_max[$i] != '*') $table_rate_max[$i] = woocommerce_format_total($table_rate_max[$i]);//number_format($table_rate_max[$i], 2,  '.', '');
							if($table_rate_excluded[$i] != 'on') $table_rate_excluded[$i] = 'off';
							...
							$table_rates[$i] = array(
								...
								'class_priority' => $class_priorities[ sanitize_title($table_rate_class[ $i ]) ]['priority'],
					...
					//$handling_rates = $this->m_array_sort($handling_rates, array('zone_order','fee','percent'));
					// Obtain a list of columns
			    ...
			    function is_available( $package ) {
					...
					return apply_filters( $this->id . '_is_available', true );
			    }
				...
				function be_zone_update_notice() {
					...
					if( !isset( $current_zones ) || $current_zones == '' || ( is_array($current_zones) && count( $current_zones ) == 0 ) ) {
						...
						if( $old_version <= 3.2 ) {
							...
							if( count( $findTable ) ) {
								echo '<div class="error" style="font-weight:bold;"><p>ATTENTION: Version 3.3 of the Table Rate Shipping plugin for WooCommerce introduced a new, more efficient way of storing zone information. You will need to update your system in order for the plugin to continue functioning properly. <a href="'.admin_url( 'admin.php?page=woocommerce_settings&tab=shipping_zones&upgrade=zones' ).'"><button>Upgrade Zones</button></a></p></div>';
							}
						} else
							echo '<div class="error" style="font-weight:bold;"><p>ATTENTION: You have not defined any shipping zones for the WooCommerce Table Rate Shipping plugin. You must setup your zones before creating any rates in the shipping method\'s settings page. <a href="'.admin_url( 'admin.php?page=woocommerce_settings&tab=shipping_zones' ).'"><button>Setup Zones</button></a></p></div>';
					}
				}
				...
				function install_plugin_button() {
					...
					wp_safe_redirect( admin_url( 'admin.php?page=woocommerce_settings&tab=shipping_zones&upgrade=true' ) );
					exit;
				}
...
function ts_add_plugin_action_links( $links ) {
	return array_merge(
		array(
			'settings' => '<a href="' . get_bloginfo( 'wpurl' ) . '/wp-admin/admin.php?page=woocommerce_settings&tab=shipping&section=BE_Table_Rate_Shipping">Settings</a>'
		),
		$links
	);
